...
// Namespace added to protect global namespace
namespace sgp4
{
...
void twoline2rv(char longstr1[130], char longstr2[130],
                gravconsttype whichconst, elsetrec &satrec)
{
    ...
    if (dummy == 0) {
        std::cerr << "Error parsing first line of TLE." << std::endl;
    }

    if (longstr2[52] == ' ') {
        dummy = sscanf(
            longstr2, "%2d %5ld %9lf %9lf %8lf %9lf %9lf %10lf %6ld \n",
            &cardnumb, &satrec.satnum, &satrec.inclo, &satrec.nodeo,
            &satrec.ecco, &satrec.argpo, &satrec.mo, &satrec.no, &revnum);
    } else {
        dummy = sscanf(
            longstr2, "%2d %5ld %9lf %9lf %8lf %9lf %9lf %11lf %6ld \n",
            &cardnumb, &satrec.satnum, &satrec.inclo, &satrec.nodeo,
            &satrec.ecco, &satrec.argpo, &satrec.mo, &satrec.no, &revnum);
    }
    if (dummy == 0) {
        std::cerr << "Error parsing second line of TLE." << std::endl;
    }
    ...
    char opsmode = 'i'; // Use "improved" mode of operation

    // ---------------- initialize the orbit at sgp4epoch -------------------
    sgp4init(whichconst, opsmode, satrec.jdsatepoch - 2433281.5,
