...
spacecraft area finds a clear minimum at 34 m^2 against a mass of 1630 kg. Say
carefully what that is: the model carries C_R = 1.1444 as well, so what the
sweep determines is A\*C_R/m = 0.0239 m^2/kg, not an area over a mass. Against a
published GPS geometric A/m of about 0.021 that implies an effective C_R near
1.14, which is a reasonable value for the spacecraft - so the physical check
holds, it just has to be stated as one quantity against another rather than as
a number against itself.

The check that needs no such care is internal: the seven-parameter fit, which
tunes no area at all, gives A\*C_R/m = 0.0236. The two agree to one percent, and
both carry the same C_R, so the agreement is a ratio and survives any argument
...
A/m of 6.36. Those two do not divide: the left has the radiation coefficient
inside it and the right does not, so 3.36/6.36 = 0.53 is the fraction of the
sail only if C_R = 1, which is the one assumption a mirror does not deserve.
What the ratio actually gives is f\*C_R = 0.53, and f needs the sail's own
coefficient.

For a specular flat plate at incidence angle theta the force is 2\*P\*A\*cos^2
along the normal, so its component down the sun line is 2\*P\*A\*cos^3 and a
cannonball fitted through it recovers an equivalent A\*C_R of 2\*A\*cos^3. A sail
alternating face-on and edge-on with duty cycle d therefore averages 2\*d\*A, and

    d = 3.36 / (2 * 6.36) = 26%

**which is what the drag side independently says**: an effective C_D of 0.52 on
32 m^2 is 16.6 m^2, or 7.6 m^2 at a physical 2.2, 24% of the sail. Two numbers
from two different forces, agreeing to about ten percent, both saying the sail
spends roughly three-quarters of its time side-on. With a real aluminised
reflectivity of about 0.88 rather than a perfect mirror the radiation figure is
28% instead of 26%, which does not change the story.

This is worth spelling out because the first version of this paragraph divided
3.36 by 6.36, reported 53%, and called it agreement with the drag figure of
24%. It was not agreement; it was a factor of 2.2 asserted across. The
correction is what makes the two measurements actually meet - and it is the same
apples-to-oranges the C_R defect below was about, committed again one paragraph
later, in a document describing the fix.
...
- **`effective A*C_R/m` was not A\*C_R/m.** Both fits reported the estimated
  ...
  than 0.02064 m^2/kg. The two are different quantities, and anything comparing
  them to a published figure has to say which: 0.02362 is A\*C_R/m and belongs
  against a geometric A/m times a coefficient, while 0.02064 is that divided by
  the model's own C_R and means nothing on its own.
