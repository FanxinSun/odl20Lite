...
void Force_drag_tiegcm::compute_acceleration() {
  ...
  // for details
  if (state->geodetic.alt < 360.0 || state->geodetic.alt > 413.0) {
  ...
  if (state->eci.epoch.get_MJD_UTC() < 58281.0 ||
      state->eci.epoch.get_MJD_UTC() > 58283.9895833) {
    std::stringstream error;
    error.precision(16);
  ...
  if ((state->geodetic.lat) * 180 / M_PI < -58.76 ||
      (state->geodetic.lat) * 180 / M_PI > 56.24) {
    std::stringstream error;
    error.precision(16);
...
void Force_drag_tiegcm::loadfiles() {
  std::string Densityfilepath = "../res/Densities4854.txt";
  ...
  if (in_densityfile.is_open()) {
    ...
    std::cout << "Force_drag_tiegcm: could not open " << Densityfilepath
              << " - TIE-GCM densities are unavailable. Run from the bin/"
                 " directory so the ../res path resolves." << std::endl;
  }

  std::string Heightfilepath = "../res/Heights4854.txt";
  ...
  if (in_heightfile.is_open()) {
    ...
    std::cout << "Force_drag_tiegcm: could not open " << Heightfilepath
              << " - TIE-GCM heights are unavailable. Run from the bin/"
                 " directory so the ../res path resolves." << std::endl;
  }
}
...
double Force_drag_tiegcm::get_density(double time, double altitude,
  ...
  // intervals passed
  double time1 = floor(((time - 58281) * 24 * 60) / 15) + 1;
  // cout << "lo1 is " << lo1 << ", la1 is " << la1 << " and time1 is: " <<
  // time1 << " " << endl; cout<< longitude << " :lo1: " << lo1 << ", " <<
  // latitude <<" :la1: " <<la1 <<", "<< altitude << " :altitude: " << altitude
  // <<", "<< time << " :time: " << time1 << endl; cout <<
  // Densityvalues(lo1,la1)
  // << endl;
  if (time1 < 0) {
    time1 = 1;
  }
  ...
  for (double i = time1; i <= time1 + 1; i++) {
    ...
    for (double lo = lo1; lo <= lo1 + 1; lo++) {
      ...
      for (double la = la1; la <= la1 + 1; la++) {
        ...
        // cout << Density << endl;
        // cout << Height << endl;
        // The altitude scan below walks rows ln..ln+8 of column lo. Skip any
        // corner that falls outside the loaded grid; the V.size() test after
        // these loops turns a short corner set into a clean diagnostic.
        if (ln < 0 || lo < 0) {
          continue;
        }
        const size_t row = static_cast<size_t>(ln);
        const size_t col = static_cast<size_t>(lo);
        if (row + 9u > heightfile.size() || row + 9u > densityfile.size() ||
            col >= heightfile[row].size() || col >= densityfile[row].size()) {
          continue;
        }

        double Height = heightfile[row][col];
        double Density = densityfile[row][col];
        // cout << Height << endl;
        double lowheight;
        double highheight;
        double lowdensity;
        ...
        // the height value is not contained within the height database
          ...
          std::cout << "ERROR: the height value: " << altitude
                    << "km is not contained within the database "
                       "'Heights.txt', check raw data around row number: "
                    << ln << " and column number: " << lo1 << std::endl;
        }

        for (int j = 1; j < 8; j++) {
          Height = heightfile[row + static_cast<size_t>(j)][col];
          // cout << Height << endl;
          Density = densityfile[row + static_cast<size_t>(j)][col];
          if (Height < altitude) {
            lowheight = Height;
            lowdensity = Density;
          } else if (Height >= altitude) {
            ...
            //std::cout << lowheight << " & " << highheight << std::endl;
            //std::cout << lowdensity << " & " << highdensity << std::endl;
  ...
  // interpolate() reads the eight grid corners v[0]..v[7] unconditionally, so
  // a short set means the requested point lies outside the loaded TIE-GCM data
  // (altitude outside the height table, latitude beyond the modelled band, or
  // an epoch outside the window the files cover). Report it once and fall back
  // to zero density instead of indexing past the end of V.
  if (V.size() < 8) {
    if (!reported_out_of_range) {
      reported_out_of_range = true;
      std::cout.precision(10);
      std::cout << "Force_drag_tiegcm: requested point lies outside the "
                   "TIE-GCM data set; using zero density from here on where "
                   "that is the case."
                << "\n                   MJD UTC   : " << time
                << "\n                   altitude  : " << altitude / 100000.0
                << " km"
                << "\n                   latitude  : " << latitude << " deg"
                << "\n                   longitude : " << longitude << " deg"
                << "\n                   grid corners found: " << V.size()
                << " of 8"
                << "\n                   The bundled res/Heights4854.txt and "
                   "res/Densities4854.txt cover 12-15 June 2018 (MJD 58281+), "
                   "roughly 342-481 km altitude and +-58 deg latitude."
                << "\n                   Use drag = 1 (USSA76) for orbits "
                   "outside that envelope." << std::endl;
    }
    return 0.0;
  }
...
double Force_drag_tiegcm::interpolate(double latitude, double longitude,
  ...
  double realtime = ((time - 58281) * 24 * 60) / 15;
  double time1 = floor(((time - 58281) * 24 * 60) / 15);
  // cout << longitudelower << "   " << latitudelower << endl;
