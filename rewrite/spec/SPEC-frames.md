...
**Confirmed against `ARN15`, 2026-09-24 (`FRAME-Q-002` CLOSED).** The definition above was
originally stated from first principles because neither primary source (`BEU94`, `ARN15`) could
be obtained; `ARN15` is now obtained (authors' accepted manuscript, green OA via CORE — see §2).
His own Eq. 1, quoted: *"e_D := (r_s − r)/|r_s − r|, e_Y := −(e_r × e_D)/|e_r × e_D|, e_B :=
e_D × e_Y"*, where r_s, r are the geocentric Sun/satellite vectors and e_r = r/|r|. e_D's own
sense (spacecraft → Sun) matches `FRAME-R-050` directly. e_Y matches `FRAME-R-051` too, though
not by identical notation: Arnold's −(e_r×e_D) and this spec's (ê_D×ê_r) are the SAME vector —
cross-product anti-commutativity, a×b = −(b×a), makes −(e_r×e_D) = e_D×e_r exactly, checked
symbolically, not asserted from the resemblance alone. `BEU94` (Beutler 1994) remains unobtained,
but nothing here was shown to depend on it once `ARN15` alone settles both conventions.
...
| `FRAME-Q-002` | **CLOSED, 2026-09-24, on `ARN15`'s own Eq. 1.** `BEU94` and `ARN15`, the primary sources for the DYB frame, could not be obtained at the time this was raised — the 1994 paper is in a journal with no accessible archive, and the 2015 paper was paywalled at Springer (abstract only). §4.7's definition was stated from first principles, internally unambiguous, but with the **sign of ê_D** and the **construction of ê_Y** as conventions fixed rather than inherited. `ARN15` is now obtained (the authors' own accepted manuscript, green OA via CORE, `PROVENANCE.md`'s own rule-4 register) — his own Eq. 1: *"e_D := (r_s−r)/\|r_s−r\|, e_Y := −(e_r×e_D)/\|e_r×e_D\|, e_B := e_D×e_Y."* Checked, not asserted from resemblance: e_D's sense matches `FRAME-R-050` directly; e_Y matches `FRAME-R-051` once cross-product anti-commutativity is applied (−(e_r×e_D) = e_D×e_r, the identical vector to this spec's own (ê_D×ê_r)). Both conventions this entry worried about were already right. | **Done.** `BEU94` remains unobtained, but nothing in §4.7 was shown to depend on it once `ARN15` alone settles ê_D and ê_Y. `FRAME-R-050`/`-051` keep their own restate-at-every-reporting-point requirement regardless — that discipline does not depend on this question being open. |
