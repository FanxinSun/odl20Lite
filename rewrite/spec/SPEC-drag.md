...
- **DRAG-P-3.** `DRAG-R-003`'s velocity-Jacobian tensor is checked against a central finite
  ...
  without comment.
- **DRAG-P-4.** `DRAG-A-011`'s 1 × 10⁻⁴ bound is a **separation between two regimes**, not a
  margin fitted to its own passing measurement — the same rule-7 defect `DRAG-A-010`'s first draft
  already made once, avoided here on purpose. The switched stencil's own truncation error is order
  (Δ/*H*)²: ≈ 2 × 10⁻⁶ at O's cutoff (Δ = 0.1 km, *H* ≈ 42 km, `PROVENANCE.md` §28.5's own case),
  a few times larger at N2's — smaller local *H*, larger local jump. The STRADDLING stencil it
  replaces would be off, instead, by order (*J*/ρ)·(*H*/2Δ) — the jump misread as a smooth slope
  spanning the whole 2Δ stencil — of order 10⁻² at O, using `PROVENANCE.md` §28.10's own measured
  *J*/ρ. **1 × 10⁻⁴ sits between the two, roughly an order of magnitude above the switched
  stencil's own error and two orders below the straddling one it replaces** — wide enough to
  separate "the switch fired and is working" from "the switch did not fire," which is what makes
  `DRAG-A-011` a discriminator rather than a number read off its own result.
